...
export function DayOverviewHeatMap({
  ...
  useEffect(() => {
    ...
    if (ref.current !== null && Array.isArray(dayData.details) && dayData.details?.length > 0) {
      ...
      const xAxisLabels = range(0, binCountPerHour).map((e) => createXAxisLabel(e));
      ...
      // Draw parent 'g' tag
      const parent = svg.append("g").attr("transform", `translate(${margin.left}, ${margin.top})`);
      ...
      // Add text color
      selectAll(".x-axis, .y-axis").selectAll("text").attr("fill", "var(--primary_color)");

      // Add path color
      selectAll(".x-axis, .y-axis").selectAll("path").attr("stroke", "var(--primary_color)");
  ...
  }, [color, dayData, onCellClick, onHideTooltip, onTooltip, response, showXAxisLabels]);
